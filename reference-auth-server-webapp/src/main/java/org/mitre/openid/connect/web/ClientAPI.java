...
public class ClientAPI {
    ...
    public String apiShowClientByClientId(@RequestParam("clientId") String clientId, Model model, Authentication auth) {
        ...
        if (client == null) {
            logger.error("apiShowClient failed; client with clientId " + clientId + " could not be found.");
            model.addAttribute(HttpCodeView.CODE, HttpStatus.NOT_FOUND);
            model.addAttribute(JsonErrorView.ERROR_MESSAGE, "The requested client with clientId " + clientId + " could not be found.");
            return JsonErrorView.VIEWNAME;
        }
