...
public class OAuth2AccessTokenEntity implements OAuth2AccessToken {
    ...
    public Map<String, Object> getAdditionalInformation() {
        ...
        // add launch context details to the token if they exist
        if (getLaunchContext() != null) {
            for (LaunchContextEntity cparam : getLaunchContext()) {
                ...
                if (val.equals("true")) {
                    additionalInformation.put(cparam.getName(), true);
                } else if (val.equals("false")) {
                    additionalInformation.put(cparam.getName(), false);
                } else {
                    additionalInformation.put(cparam.getName(), val);
                }
            }
        }
