...
ENTRYPOINT [ "sh", "-c", "java $JAVA_OPTS -Dspring.profiles.active=users-keycloak,local -Djava.security.egd=file:/dev/./urandom -jar jetty-runner.jar --config jetty.xml app.war" ]
